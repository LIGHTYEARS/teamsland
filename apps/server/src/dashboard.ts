...
export interface DashboardServer {
  ...
  broadcastSessionUpdate: (action: WsSessionUpdate["action"], sessionId: string) => void;
}
...
export function startDashboard(deps: DashboardDeps, signal?: AbortSignal): DashboardServer {
  ...
  const unsubscribe = registry.subscribe((agents) => {
    ...
    broadcast(clients, { type: "session_update", action: "status_changed", sessionId: "" } satisfies WsSessionUpdate);
  });
  ...
  return {
    ...
    broadcastSessionUpdate(action: WsSessionUpdate["action"], sessionId: string): void {
      broadcast(clients, { type: "session_update", action, sessionId });
    },
  };
}
