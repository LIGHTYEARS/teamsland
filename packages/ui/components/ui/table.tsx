...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-9 px-3 text-left align-middle font-medium text-muted-foreground whitespace-nowrap [&:has([role=checkbox])]:pr-0 [&>[role=checkbox]]:translate-y-[2px]",
        className,
      )}
      {...props}
    />
...
function TableEmpty({ className, colSpan, children, ...props }: React.ComponentProps<"td"> & { colSpan: number }) {
  return (
    <tr>
      <td colSpan={colSpan} className={cn("h-24 text-center text-muted-foreground", className)} {...props}>
        {children}
      </td>
    </tr>
  );
